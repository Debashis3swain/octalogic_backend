"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BookingsController = void 0;
const common_1 = require("@nestjs/common");
const bookings_service_1 = require("./bookings.service");
const response_dto_1 = require("../common/response.dto");
const users_service_1 = require("../users/users.service");
let BookingsController = class BookingsController {
    constructor(bookingsService, usersService) {
        this.bookingsService = bookingsService;
        this.usersService = usersService;
    }
    async createBooking(userId, createBookingDto) {
        try {
            const user = await this.usersService.findById(userId);
            if (!user) {
                return (0, response_dto_1.createErrorResponse)({
                    message: 'User not found',
                    code: 'USER_NOT_FOUND'
                });
            }
            const booking = await this.bookingsService.createBookingFromDto(Object.assign(Object.assign({}, createBookingDto), { name: user.name, email: user.email }));
            return (0, response_dto_1.createSuccessResponse)({
                booking,
                message: 'Booking created successfully'
            });
        }
        catch (error) {
            return (0, response_dto_1.createErrorResponse)(error);
        }
    }
    async getBookingsByEmail(email) {
        try {
            const bookings = await this.bookingsService.findByUserEmail(email);
            return (0, response_dto_1.createSuccessResponse)({ bookings });
        }
        catch (error) {
            return (0, response_dto_1.createErrorResponse)(error);
        }
    }
    async getAll() {
        try {
            const bookings = await this.bookingsService.findAll();
            return (0, response_dto_1.createSuccessResponse)({ bookings });
        }
        catch (error) {
            return (0, response_dto_1.createErrorResponse)(error);
        }
    }
    async checkVehicleAvailability(vehicleId, dateRange) {
        try {
            const { startDate, endDate } = dateRange;
            const isAvailable = await this.bookingsService.checkVehicleAvailability(vehicleId, startDate, endDate);
            return (0, response_dto_1.createSuccessResponse)({
                available: isAvailable
            });
        }
        catch (error) {
            return (0, response_dto_1.createErrorResponse)(error);
        }
    }
};
exports.BookingsController = BookingsController;
__decorate([
    (0, common_1.Post)(':userId'),
    __param(0, (0, common_1.Param)('userId', common_1.ParseIntPipe)),
    __param(1, (0, common_1.Body)(new common_1.ValidationPipe({ transform: true }))),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Object]),
    __metadata("design:returntype", Promise)
], BookingsController.prototype, "createBooking", null);
__decorate([
    (0, common_1.Get)('user/:email'),
    __param(0, (0, common_1.Param)('email')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], BookingsController.prototype, "getBookingsByEmail", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], BookingsController.prototype, "getAll", null);
__decorate([
    (0, common_1.Get)('check-availability/vehicle/:vehicleId'),
    __param(0, (0, common_1.Param)('vehicleId', common_1.ParseIntPipe)),
    __param(1, (0, common_1.Body)(new common_1.ValidationPipe({ transform: true }))),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Object]),
    __metadata("design:returntype", Promise)
], BookingsController.prototype, "checkVehicleAvailability", null);
exports.BookingsController = BookingsController = __decorate([
    (0, common_1.Controller)('bookings'),
    __metadata("design:paramtypes", [bookings_service_1.BookingsService,
        users_service_1.UsersService])
], BookingsController);
//# sourceMappingURL=bookings.controller.js.map